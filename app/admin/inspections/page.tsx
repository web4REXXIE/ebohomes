...
import { Search, ClipboardCheck, Home as HomeIcon, CheckCircle2, XCircle } from 'lucide-react'
...
function InspectionsPageInner() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
      ...
      {/* Backup picker — only shown if no listing selected yet */}
      {!listing && (
              ...
              <div className="space-y-2">
                {pastInspections.map((insp) => (
                  <div key={insp.id} className="bg-card border border-border rounded-lg p-3 text-xs text-muted-foreground">
                    ...
                    {insp.overall_notes && <p className="mt-1">{insp.overall_notes}</p>}
                  </div>
                ))}
              </div>
            </div>
